    ...
    if(length(ids) > 0) {
      ...
      mults <- sapply(ids, function(id) {
        ifelse(locations$type[id] == 'tree' & height < plant_heights[id] / 2, 0,
               ifelse(locations$type[id] == 'tree' & height >= plant_heights[id] / 2, sqrt(pmax(0, (plant_heights[id] * 0.25)^2 - (height - (plant_heights[id] * 0.75))^2)) / (plant_heights[id] * 0.25),
                      ifelse(locations$type[id] == 'shrub', sqrt(pmax(0, (plant_heights[id] * 0.5)^2 - (height - (plant_heights[id] * 0.5))^2)) / (plant_heights[id] * 0.5),
