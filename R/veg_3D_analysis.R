...
process_structure <- function(heights = c(0.1, 0.2, 0.5, 1.0, 2.0, 5.0, 10.0), # Input height datums in meters
  ...
  proportion_grids <- lapply(heights, function(height) {
    ...
    if(length(ids) > 0) {
      ...
      st_write(planted_area, paste0("output/sp_vegetation/year_", sprintf("%02d", year), "_height_", height, "_veg.shp"), append = FALSE)
      ...
      # Record proportions overlap to eligible grids
      out_grid$prop_ol[overlap_idx] <- areas / st_area(grid$geometry[1])
      
    }
    
    # Write out grids for given year and selected heights
    st_write(out_grid, paste0("output/sp_grids/year_", sprintf("%02d", year), "_height_", height, "_grd.shp"), append = FALSE)
...
test <- process_structure(year = 5, locations = plant_points, obstructions = building, grid = hex_grid)
